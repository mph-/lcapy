...
def SpecialDiff( var , Q , o ,s, var_a, var_N):
    """Make derivative of a**(k-s)*Q / (1-a**N)**o with respect to a 
         AND divide result by a**(k-s-1)/(1-a**N)**(o+1) to obtain a polynom in n
         This is used with Make_Polynom_Table
    """
...
def Make_Polynom_Table_1( var, MaxP, aa=None,NN=None, prn=False ):
    ...
    for l in range(2,MaxP+1,1):
        ...
        Q0 = SpecialDiff(var,Qpoly[l-1][0],l-1,0,a,N) + Qpoly[l-1][0]*(1-a**N)*(l-1) 
        this_Qpoly =[ sym.simplify(sym.expand(Q0)) ]
        # run for each p through all 0<=mu<=p-1
        for m in range(l-1):
            Qm = SpecialDiff(var,Qpoly[l-1][m] , l-1 ,m,a,N )    
...
#
def Make_Polynom_Table_2(var,p, varN=None, prn=False):
    """  This Table contains solutions of the DFT of
            exp( j*2*pi/N*n*mu) / ( 1-a*exp( j*2*pi/N*n) )**p 
            where mu < p  and mu, p are integer
           ...
           linear combination of the transforms in (1)  (see manuscript)
    ...
    # Solve now the equation    x_p*C_p + x_(p-1)*C_(p-1)(1-q) +... = q**s 
    #                                                                                       = r_1 +r_2*q + .. r_p*q**(p-1)
    ...
    # solve for the xi by equatiing coefficients in q  
...
# e.g. exp(j*2*pi/N) = (-1)**(2/N), or denoms are expanded  and not factorized  
# or it takes to long 
def simp_rat(expr,q,method=1):
    ...
    if method == 1:
        ...
        # denominator was probabliy expanded, so return original one
        if Denom.is_Add and len(Denom.args) == 3:
            return sym.numer(expr).as_poly(q).as_expr() / sym.denom(expr)
        else:
            ret_expr = sym.numer(ret_expr).as_poly(q).as_expr() / sym.factor( Denom ,q )         
            return ret_expr
    elif method == 2:
        Num = sym.numer(expr)
        ...
        for i,xx in enumerate(f_d[1]):
            for j,yy in enumerate(f_n[1]):
                ...
                if  not hits.has(q):
                    ...
                    if p_n<=p_d:
                        f_n[1][j] = ( xx[0] , 0)
                        f_d[1][i] = ( xx[0], p_d-p_n )
                    else:
                        f_n[1][j] = ( xx[0] , p_n-p_d )
                        f_d[1][i] = ( xx[0],  0 )
                    break    
        ...
        print("Only    0<= method <=2  possible")
        return expr
...
class DFTTransformer(BilateralForwardTransformer):
    ...
    def termXq(self, expr, n, k, q, lower, upper):
        ...
        if (expr == n or
            ...
            # make the first few by hand to make expressions shorter and faster, use in all cases for the derivatives (and multiplication with q) the structure
            # ( q**lower * polynomial (q)- q**(upper+1)*polynomial(q)  ) (1-q)**(p+1) . The order of the polynomials is equal to p
            if p==1:
                A_l = lower + q*(1 - lower)
                B_u = -q*upper + upper + 1
            elif p==2:
                A_l = lower**2 + q**2*(lower**2 - 2*lower + 1) + q*(-2*lower**2 + 2*lower + 1)
                B_u =  q**2*upper**2 + q*(-2*upper**2 - 2*upper + 1) + upper**2 + 2*upper + 1
            elif  p==3:
                A_l =  lower**3 + q**3*(-lower**3 + 3*lower**2 - 3*lower + 1) + q**2*(3*lower**3 - 6*lower**2 + 4) + q*(-3*lower**3 + 3*lower**2 + 3*lower + 1)
                B_u = -q**3*upper**3 + q**2*(3*upper**3 + 3*upper**2 - 3*upper + 1) + q*(-3*upper**3 - 6*upper**2 + 4) + upper**3 + 3*upper**2 + 3*upper + 1
            # generate the polynoms A_l, B_u individually
            # TODO  additional option to make a smart simplification by inspection of upper und lower
            else:    
                # first polynoms
                A_l = 1 +0*q   
                B_u = 1+0*q   
    ...
    def termXk(self, expr, n, k):   
        ...
        qq = sym.Symbol('qq')
        p0=sym.Wild('p0',properties=[lambda r: r.is_integer and r.is_nonnegative] )    
        ...
        # replace exp( j*2*pi/N*n) by qq (forward)
        # on calling termXk for the inverse, n is k and k is n 
        expr_qq = expr.replace(sym.exp(  sg*sym.I * 2 * pi * n / self.N * p0), qq**p0)
            ...
            # denominator is D=(1-a*qq)**p   , p>=2
                ...
                if pp == 2:
                    ...
                    Q_p_mu = [ N4*a4N + 11*N4*a3N + 11*N4*a2N + N4*aN - 10*N3*a4N - 30*N3*a3N + 30*N3*a2N + 10*N3*aN + 35*N2*a4N - 35*N2*a3N - 35*N2*a2N + 35*N2*aN - 50*NN*a4N + 150*NN*a3N - 150*NN*a2N + 50*NN*aN + 24*a4N - 96*a3N + 144*a2N - 96*aN + 24 +
                                                      ...
                                                      kn*(-4*N3*a4N - 12*N3*a3N + 12*N3*a2N + 4*N3*aN + 30*N2*a4N - 30*N2*a3N - 30*N2*a2N + 30*N2*aN - 70*NN*a4N + 210*NN*a3N - 210*NN*a2N + 70*NN*aN + 50*a4N - 200*a3N + 300*a2N - 200*aN + 50)  , 
                    ...
                    # TODO  make faster 
                    # we increase performace for pp>=6  by saving the table in an extra file and read the polynoms and subs 
                    # or make Make_Polynom_Table_1 faster by substututing a**(r*N) by arN and so on
                    Q_p_mu = Make_Polynom_Table_1( kn, pp, aa, NN)
        ...
        if expr.is_Piecewise:
            match=list( expr.args[0][1].find(sym.Ne(q,p0)) )
            n0=match[0].args[1]
                ...
                # denominator is D=(1-a*qq)**p   , p>=2
                    ...
                    if pp == 2:                
                        ...
                        T_p_mu = [ (NN**4 - 110*NN**2 + 360*NN - 30*kn**4 + kn**3*(60*NN - 240) + kn**2*(-30*NN**2 + 360*NN - 660) + kn*(-120*NN**2 + 660*NN - 720) - 251) / 720, 
                                             ...
                                             (NN**4 + 10*NN**2 + 60*NN*kn**3 - 60*NN*kn - 30*kn**4 + kn**2*(60 - 30*NN**2) - 11) / 720, 
                                             (NN**4 - 20*NN**2 - 30*kn**4 + kn**3*(60*NN + 120) + kn**2*(-30*NN**2 - 180*NN - 120) + kn*(60*NN**2 + 120*NN) + 19) / 720 ]
                    elif pp==5:
                        T_p_mu =  [ (5*NN**4 - 250*NN**2 + 720*NN - 12*kn**5 + kn**4*(30*NN - 150) + kn**3*(-20*NN**2 + 300*NN - 700) + kn**2*(-150*NN**2 + 1050*NN - 1500) + kn*(2*NN**4 - 350*NN**2 + 1500*NN - 1440) - 475) / 1440, 
                                              ...
                                              (NN**4 + 10*NN**2 - 12*kn**5 + kn**4*(30*NN - 30) + kn**3*(-20*NN**2 + 60*NN + 20) + kn**2*(-30*NN**2 - 30*NN + 60) + kn*(2*NN**4 + 10*NN**2 - 60*NN) - 11) / 1440, 
                                              (-NN**4 - 10*NN**2 - 12*kn**5 + kn**4*(30*NN + 30) + kn**3*(-20*NN**2 - 60*NN + 20) + kn**2*(30*NN**2 - 30*NN - 60) + kn*(2*NN**4 + 10*NN**2 + 60*NN) + 11) / 1440, 
                                              (-3*NN**4 + 30*NN**2 - 12*kn**5 + kn**4*(30*NN + 90) + kn**3*(-20*NN**2 - 180*NN - 220) + kn**2*(90*NN**2 + 330*NN + 180) + kn*(2*NN**4 - 110*NN**2 - 180*NN) - 27) / 1440 ]
                    else:
                        T_p_mu = Make_Polynom_Table_2(kn,pp,varN=NN)
                    ...
                    result = 0
                    for i in range( len(num_coeff) ):
                        result += num_coeff[i] * T_p_mu[i] * sym.exp(j*2*pi*n0/self.N*i*sg) 
                    result *= sym.exp(-j*2*pi*n0/self.N*k*sg) / c0**pp                       
