...
class LcapyTester(unittest.TestCase):
    ...
    def test_ztransform(self):
        ...
        self.assertEqual( ((2 + 3*j)**n).ZT() ,-z/(-z + 2 + 3*j) ,"(2 + 3*j)**n")
        self.assertEqual( (exp(n*(3 + 2*j))).ZT() ,z/(z - exp(3 + 2*j)) ,"exp(n*(3 + 2*j))")
        self.assertEqual( (n*(2 + 3*j)**n).ZT() ,z*(2 + 3*j)/(-z + 2 + 3*j)**2 ,"n*(2 + 3*j)**n")
        self.assertEqual( (n*exp(n*(3 + 2*j))).ZT() ,z*exp(3 + 2*j)/(z - exp(3 + 2*j))**2 ,"n*exp(n*(3 + 2*j))")
        self.assertEqual( (n**2*(2 + 3*j)**n).ZT() ,-z*(2 + 3*j)*(z + 2 + 3*j)/(-z + 2 + 3*j)**3 ,"n**2*(2 + 3*j)**n")
        self.assertEqual( (n**2*exp(n*(3 + 2*j))).ZT() ,z*(z + exp(3 + 2*j))*exp(3 + 2*j)/(z - exp(3 + 2*j))**3 ,"n**2*exp(n*(3 + 2*j))")
        self.assertEqual( (2**n*exp(3*j*n)).ZT() ,z/(z - 2*exp(3*j)) ,"2**n*exp(3*j*n)")
        self.assertEqual( (2**n*n*exp(3*j*n)).ZT() ,2*z*exp(3*j)/(z - 2*exp(3*j))**2 ,"2**n*n*exp(3*j*n)")
        self.assertEqual( (3**n*n**2*exp(4*j*n)).ZT() ,3*z*(z + 3*exp(4*j))*exp(4*j)/(z - 3*exp(4*j))**3 ,"3**n*n**2*exp(4*j*n)")        
    ...
    def test_inverse_ztransform(self):
        ...
        self.assertEqual( (-z*(-2*z + 2 + exp(2))/((z - 2)*(z - exp(2)))).IZT(causal=True) ,-(-2**n - exp(2*n))*UnitStep(n) ,"-z*(-2*z + 2 + exp(2))/((z - 2)*(z - exp(2)))")
        self.assertEqual( (z*(1 + 2*j)*(3*z*(z + 1 + 2*j) + (-z + 1 + 2*j)*(2*z + 1 + 2*j))/(-z + 1 + 2*j)**4).IZT(causal=True,) ,(1 + 2*j)*(1 + 2*j)**n*(n*(-18 + 24*j)*(n - 2)*(n - 1)/(6*(1 + 2*j)**3) + n*(6 + 12*j)*(n - 1)/(2*(1 + 2*j)**2) + n*(1 - 2*j)/5)*UnitStep(n) ,"z*(1 + 2*j)*(3*z*(z + 1 + 2*j) + (-z + 1 + 2*j)*(2*z + 1 + 2*j))/(-z + 1 + 2*j)**4")
        self.assertEqual( (z*(6*z + 5 - exp(3 + 4*j))/((z + 1)*(z - exp(3 + 4*j)))).IZT(causal=True) ,((-1)**n + 5*exp(3 + 4*j)**n)*UnitStep(n) ,"z*(6*z + 5 - exp(3 + 4*j))/((z + 1)*(z - exp(3 + 4*j)))")
        self.assertEqual( (z*((z - 6)**4*(z + 1) + (z - 1)**3*(18*z*(z + 6) - 12*(z - 6)*(z + 3)))/((z - 6)**4*(z - 1)**3)).IZT(causal=True) ,(6**n*(n*(n - 2)*(n - 1) + 3*n*(n - 1) + n) + n*(n - 1) + n)*UnitStep(n) ,"z*((z - 6)**4*(z + 1) + (z - 1)**3*(18*z*(z + 6) - 12*(z - 6)*(z + 3)))/((z - 6)**4*(z - 1)**3)")
        self.assertEqual( (4*z*(18*z - 5)/((2*z - 1)*(4*z - 1))).IZT(causal=True,) ,4*(4**(-n)/4 + 2*2**(-n))*UnitStep(n) ,"4*z*(18*z - 5)/((2*z - 1)*(4*z - 1))")
        ...
        self.assertEqual( (z*(z**2 - exp(4))*exp(5)*sin(2)/(z**2 - 2*z*exp(2)*cos(2) + exp(4))**2).IZT(causal=True, pairs=True) ,n*exp(3)*exp(2*n)*sin(2*n)*UnitStep(n) ,"z*(z**2 - exp(4))*exp(5)*sin(2)/(z**2 - 2*z*exp(2)*cos(2) + exp(4))**2")
        ...
        self.assertEqual( (2*z*exp(3*j)/(z - 2*exp(3*j))**2).IZT(causal=True) ,n*(2*exp(3*j))**n*UnitStep(n) ,"2*z*exp(3*j)/(z - 2*exp(3*j))**2")
