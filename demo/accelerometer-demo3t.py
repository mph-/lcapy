from mcircuit import V, R, L, C, IdealTransformer, Shunt, Series, TSection
import numpy as np
...
a1 = TSection(L(m2p), L(m1p), C(Sm) + R(Rm)).chain(IdealTransformer(1 / alpha)).chain(Shunt(C(C0)))


# Wood compliance approximating radiation impedance 
Smw = 725e-9

a2 = TSection(L(m2p) | (C(Smw) + R(10)), L(m1p), C(Sm) + R(Rm)).chain(IdealTransformer(1 / alpha)).chain(Shunt(C(C0)))

f = np.logspace(1, 5, 1000)
Av1 = a1.Vgain12.frequency_response(f)
AvdB1 = 20 * np.log10(abs(Av1))
Av2 = a2.Vgain12.frequency_response(f)
AvdB2 = 20 * np.log10(abs(Av2))

fig = figure()
ax = fig.add_subplot(111)
ax.semilogx(f, AvdB1, linewidth=2, label='unloaded')
ax.semilogx(f, AvdB2, linewidth=2, label='loaded')
ax.set_xlabel('Frequency (Hz)')
ax.set_ylabel('Response (V/N dB)')
ax.legend()
ax.grid(True)


t = np.linspace(0, 10e-3, 1000)
av1 = a1.Vgain12.transient_response(t)
av2 = a2.Vgain12.transient_response(t)


fig = figure()
ax = fig.add_subplot(111)
ax.plot(t * 1e3, av1, linewidth=2, label='unloaded')
ax.plot(t * 1e3, av2, linewidth=2, label='loaded')
